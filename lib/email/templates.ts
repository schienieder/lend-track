...
function interestRowHtml(data: LoanReminderData): string {
  if (data.isFixedInterest) {
    return `<tr>
                  <td style="padding: 8px 0; font-weight: 600; color: #666;">Interest Amount</td>
                  <td style="padding: 8px 0; text-align: right; color: #333;">${formatCurrency(data.fixedInterestAmount || 0, data.currency)}</td>
                </tr>`;
  }
  return `<tr>
                  <td style="padding: 8px 0; font-weight: 600; color: #666;">Interest Rate</td>
                  <td style="padding: 8px 0; text-align: right; color: #333;">${data.interestRate}%</td>
                </tr>`;
}

function interestRowText(data: LoanReminderData): string {
  if (data.isFixedInterest) {
    return `- Interest Amount: ${formatCurrency(data.fixedInterestAmount || 0, data.currency)}`;
  }
  return `- Interest Rate: ${data.interestRate}%`;
}
...
export function getDueDateReminderTemplate(data: LoanReminderData): { subject: string; html: string; text: string } {
  const { borrowerName, lenderName, principalAmount, totalAmount, dueDate, currency, daysUntilDue, isMonthlyPayment } = data;

  // For monthly payment reminders, show the current month's payment context
  ...
  if (isMonthlyPayment) {
    subject = `Monthly Payment Reminder: Your loan payment for ${currentMonthLabel} is due`;
    subjectLine = `Monthly Payment Due — ${currentMonthLabel}`;
  } else if (daysUntilDue === 0) {
    subject = `Payment Reminder: Your loan payment is due today`;
    subjectLine = 'Payment Due Today';
  } else if (daysUntilDue === 1) {
    subject = `Payment Reminder: Your loan payment is due tomorrow`;
    subjectLine = 'Payment Due Tomorrow';
  } else {
    subject = `Payment Reminder: Your loan payment is due in ${daysUntilDue} days`;
    subjectLine = `Payment Due in ${daysUntilDue} Days`;
  }
      ...
      <div class="email-wrapper" style="background-color: #e8e8e8; padding: 40px 20px;">
        <div class="email-container" style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.1);">
          ...
          <div class="content" style="padding: 30px; line-height: 1.6; color: #333;">
            ...
            <p style="margin: 0 0 15px 0;">${isMonthlyPayment
              ? `This is a friendly reminder that your monthly loan payment to <strong>${lenderName}</strong> for <strong>${currentMonthLabel}</strong> is due.`
              : `This is a friendly reminder that your loan payment to <strong>${lenderName}</strong> is due ${daysUntilDue === 0 ? '<strong>today</strong>' : daysUntilDue === 1 ? 'tomorrow' : `in <strong>${daysUntilDue} days</strong>`}.`
            }</p>
            ...
            ${isMonthlyPayment || (daysUntilDue !== undefined && daysUntilDue <= 3) ? `
  ...
  const text = `
${isMonthlyPayment ? 'Monthly Payment Reminder' : 'Payment Reminder'}

Dear ${borrowerName},

${isMonthlyPayment
  ? `This is a friendly reminder that your monthly loan payment to ${lenderName} for ${currentMonthLabel} is due.`
...
export function getOverdueReminderTemplate(data: LoanReminderData): { subject: string; html: string; text: string } {
  const { borrowerName, lenderName, principalAmount, totalAmount, dueDate, currency, daysOverdue } = data;
...
export function getLoanCreatedTemplate(data: LoanReminderData & { paymentSchedule: string }): { subject: string; html: string; text: string } {
  const { borrowerName, lenderName, principalAmount, totalAmount, dueDate, currency, paymentSchedule } = data;
...
export function getCustomReminderTemplate(data: LoanReminderData & { customMessage?: string }): { subject: string; html: string; text: string } {
  const { borrowerName, lenderName, principalAmount, totalAmount, dueDate, currency, customMessage } = data;
